...
Transform::Transform() {
    Transform(Vector4(1, 0, 0, 0), Vector4(0, 1, 0, 0), Vector4(0, 0, 1, 0));
}

Transform::Transform(const Vector4& x, const Vector4& y, const Vector4& z) : x(x), y(y), z(z) {}
...
Transform operator*(const float& alpha, const Transform& matrix) {
    return Transform(alpha * matrix.x, alpha * matrix.y, alpha * matrix.z);
}
...
Vector4 operator*(const Transform& matrix, const Vector4& vector) {
...
Transform Transform::operator+(const Transform& matrix) const {
    return Transform(this->x + matrix.x, this->y + matrix.y, this->z + matrix.z);
}
...
Matrix3 Transform::get_ortho_transform()
{
    return Matrix3(Vector3(x.x, x.y, x.z), Vector3(y.x, y.y, y.z), Vector3(z.x, z.y, z.z));
}

/*float Transform::determinant(const Transform& matrix) {
    return
...
//Transform Transform::inv(const Transform& matrix) {
