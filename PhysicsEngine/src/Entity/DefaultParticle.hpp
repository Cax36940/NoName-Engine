...
public:
	...
	DefaultParticle(const DefaultParticle& dp) : 
		...
		sprite(dp.sprite), 
		collider(dp.collider.get_size(), &particle, nullptr, nullptr){
	...
	DefaultParticle& operator=(const DefaultParticle& dp) {
		...
		sprite = dp.sprite;
		sprite.set_position(particle.get_position_ptr());
		collider = SphereCollider(dp.collider.get_size(), &particle, nullptr, nullptr);
		return *this;
	}
	...
	DefaultParticle(const Particle& _particle, const Sphere& sphere) : 
		particle(_particle),
		sprite(sphere), 
		collider(sprite.get_size(), &particle, nullptr, nullptr) {
		sprite.set_position(particle.get_position_ptr());
	}
