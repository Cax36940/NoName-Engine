#include "DampedDoubleRigidSpringComponent.hpp"
#include "System/ParticleForceRegistry.hpp"
#include "System/CollisionsRegistry.hpp"

#define MAX_EXPAND_COEF 1.5

DampedDoubleRigidSpringComponent::DampedDoubleRigidSpringComponent(Particle* first, Particle* second, const float& stiffness, const float& damping, const float& default_length) :
	first(first), 
	second(second),
	first_force(stiffness, damping, default_length),
	second_force(stiffness, damping, default_length){}

Vector3 DampedDoubleRigidSpringComponent::get_first_position() const
{
	return first->get_position();
}

Vector3 DampedDoubleRigidSpringComponent::get_second_position() const
{
	return second->get_position();
}

void DampedDoubleRigidSpringComponent::register_physics()
{
